...
int main() {
    ...
    int processArray[numberOfProcess][6];
    ...
    while(1) {
        ...
        int lowestBurst = 999, executableProcess = -1;
        for (int j = 0; j < numberOfProcess; j++) {
            if (processArray[j][2] <= 0) continue;

            if ((processArray[j][2] < lowestBurst) && (processArray[j][0] <= time)) {
                lowestBurst = processArray[j][2];
                executableProcess = j;
            }
        }
