...
int main() {
    ...
    for(int i = 0; i < 5; i++) pthread_create(&Far[i], NULL, (void *)Farmer, (void *)&a[i]);
    for(int i = 0; i < 5; i++) pthread_create(&Sho[i], NULL, (void *)ShopOwner, (void *)&a[i]);
    for(int i = 0; i < 5; i++) pthread_join(Far[i], NULL);
    for(int i = 0; i < 5; i++) pthread_join(Sho[i], NULL);
